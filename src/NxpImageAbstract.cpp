...
namespace nxpbc {
  ...
  /*NxpImageAbstract::NxpImageAbstract(uint16_t (&rawImage)[TFC_CAMERA_LINE_LENGTH]) :
          thresholdedImage_{0},
          max_{COLOR_BLACK},
          min_{COLOR_WHITE} {
  }*/
  ...
  /*void NxpImageAbstract::setRawImage(uint16_t (&rawImage)[TFC_CAMERA_LINE_LENGTH]) {
      std::memcpy(rawImage_, rawImage, sizeof(rawImage));
      process();
  }*/
  
  void NxpImageAbstract::computeMinMax(const uint16_t (&img)[TFC_CAMERA_LINE_LENGTH]) {
    max_ = COLOR_BLACK;
    //min_ = COLOR_WHITE;
    min_ = COLOR_WHITE_ORIGINAL;
    for (int i = BLACK_COUNT; i < TFC_CAMERA_LINE_LENGTH - BLACK_COUNT; i++) {
  ...
  void NxpImageAbstract::cut(
    ...
    for (int col = TFC_CAMERA_LINE_LENGTH - 1; col > TFC_CAMERA_LINE_LENGTH - 1 - BLACK_COUNT; col--) {
      srcImg[col] = max_;
    }
  ...
  uint16_t NxpImageAbstract::getAverageThreshold(const uint16_t (&srcImg)[TFC_CAMERA_LINE_LENGTH]) {
    long sum = 0;
    for (int i = BLACK_COUNT; i < TFC_CAMERA_LINE_LENGTH - BLACK_COUNT; i++) {
      sum += srcImg[i];
    }
    auto avg = static_cast<uint16_t>(sum / TFC_CAMERA_LINE_LENGTH - (2 * BLACK_COUNT));
    return avg;
  }
  
  void
  NxpImageAbstract::threshold(const uint16_t (&srcImg)[TFC_CAMERA_LINE_LENGTH], uint16_t (&dstImg)[TFC_CAMERA_LINE_LENGTH]) {
    for (int i = 0; i < TFC_CAMERA_LINE_LENGTH; i++) {
  ...
  void NxpImageAbstract::fastMedianBlur(
      ...
      uint16_t (&dstImg)[TFC_CAMERA_LINE_LENGTH],
      const int pixels) {
    std::memcpy(dstImg, srcImg, TFC_CAMERA_LINE_LENGTH);
    
    const int buffSize = pixels * 2 + 1;
    std::vector<uint16_t> blurBuffer;
    
    for (int i = ((TFC_CAMERA_LINE_LENGTH / 2) % pixels) + pixels; i < TFC_CAMERA_LINE_LENGTH - pixels; i += buffSize) {
      for (int j = -pixels; j <= pixels; j++) {
        blurBuffer.emplace_back(srcImg[i + j]);
      }
  ...
  void NxpImageAbstract::printImg(uint16_t (&img)[TFC_CAMERA_LINE_LENGTH]) {
